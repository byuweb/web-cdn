...
Most of the time, we want users to be able to automatically get updates to our library without having to change
the URL they reference. For this reason, the CDN uses 'aliases'.

There are four alias patterns used by the CDN:

### `latest` - latest production release

`latest` always points to the latest available version of your code. If you have published tags with Semantic Version names,
...
Let's say that your project has the following tags and branches:
