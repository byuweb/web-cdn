...
module.exports = function buildFilesystem(config, contentPath, workPath) {
                ...
                return new Promise((resolve, reject) => {
                    ...
                    let moves = r.rename.map(rename => {
                        ...
                        return _promiseKlaw(dest, {filter: f => path.basename(f) !== '.git-sha'})
                            ...
                            .then(files => files.map(f => {return {from: f, to: f.replace(rgx, to)};}))
                            .then(toCopy => {
                                return Promise.all(
                                    toCopy.map(f => {
    ...
    function _contentDir(library, version) {
        if (version.experimental) {
            return path.join(contentPath, library.id, constants.VERSION_ALIASES.EXPERIMENTAL_PREFIX, version.name);
        } else {
